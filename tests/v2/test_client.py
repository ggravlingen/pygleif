...
def test_v2_async_relationship_helpers_use_owns_and_ownedby() -> None:
    """aowners/achildren should map to owns/ownedBy filters."""
    transport = FakeTransport({"lei-records": {"meta": {}, "data": []}})
    client = GleifClient(transport=transport)
    asyncio.run(client.aowners("315700WQBDF1ZVVE0T64"))
    asyncio.run(client.achildren("529900IYQRX2JSSIZA36"))
    assert transport.calls[0][1]["filter[owns]"] == "315700WQBDF1ZVVE0T64"
    assert transport.calls[1][1]["filter[ownedBy]"] == "529900IYQRX2JSSIZA36"
